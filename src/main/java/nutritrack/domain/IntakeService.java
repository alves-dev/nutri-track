...
import nutritrack.domain.liquid.LiquidIntake;
import nutritrack.domain.liquid.LiquidType;
import nutritrack.domain.liquid.LiquidTypesConfig;
import nutritrack.domain.solid.NutrientWeight;
import nutritrack.domain.solid.SolidIntake;
import nutritrack.event.v1.EventLiquid;
import nutritrack.event.v1.EventLiquidAcceptableV1;
import nutritrack.event.v1.EventLiquidSummaryV1;
import nutritrack.event.v3.EventHealthNutritionMealsV1;
import nutritrack.rabbit.RabbitMQProducer;
...
public class IntakeService {
    ...
    public void processEventHealthNutritionMealsV1(@Observes(during = AFTER_SUCCESS) final EventHealthNutritionMealsV1 event) {
        LocalDateTime localDateTime = event.getData().datetime().withZoneSameInstant(ZoneId.of(TIME_ZONE)).toLocalDateTime();
